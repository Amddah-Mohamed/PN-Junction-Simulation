...
%% ---------------- Boucle de convergence PN à l'équilibre ---------------- %%
...
colors = lines(length(Tau_list)); % Générer un ensemble de couleurs distinctes
...
for i=1:length(Tau_list)
    ...
    for j=1:length(Vapp_plus)
        ...
        while (Erreur > SetErr)
            ...
            [n, p, rho] = Charge_Bernoulli(V_sol, X,n,p,Tau_list(i));   % Mise à jour des profils de charge avec la fonction de Bernoulli
    ...
    title('Caractéristique I(V) pour différentes valeurs de \Tau');
    grid on;
